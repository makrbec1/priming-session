...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Documents\\MSc\\sleepysleepy\\programming_group\\priming-session\\priming.py',
...
for thisNeg_trial in neg_trials:
    ...
    for thisComponent in ratingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store data for neg_trials (TrialHandler)
    neg_trials.addData('rating_2.response', rating_2.getRating())
    neg_trials.addData('rating_2.rt', rating_2.getRT())
    neg_trials.addData('rating_2.started', rating_2.tStart)
    neg_trials.addData('rating_2.stopped', rating_2.tStop)
    neg_trials.addData('text_3.started', text_3.tStartRefresh)
    neg_trials.addData('text_3.stopped', text_3.tStopRefresh)
    # the Routine "rating" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
